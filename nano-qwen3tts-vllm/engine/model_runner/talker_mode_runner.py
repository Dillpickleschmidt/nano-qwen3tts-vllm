...
class TalkerModeModelRunner(ModelRunner):
    ...
    def run(self, seqs: list[Sequence], is_prefill: bool) -> list[int]:
        ...
        if self.rank == 0:
            rep_penalties = torch.tensor([seq.repetition_penalty for seq in seqs], dtype=torch.float32, device="cuda")
            prev_ids = [seq.prev_token_ids for seq in seqs]
            token_ids = self.sampler(logits, temperatures, repetition_penalties=rep_penalties, prev_token_ids=prev_ids).tolist()
        else:
            token_ids = None
        reset_context()
